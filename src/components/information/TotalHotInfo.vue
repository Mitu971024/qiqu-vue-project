<template>
  <div class="total">
    ...
    <div v-for="(hotinfo,index) in totalhotinfo" class="onediv" v-if="index<data">
      <figure>
        <h4 class="detail"  @click="tohotdetails(hotinfo.zId)"><span>| 热门资讯 |&nbsp;&nbsp;</span>{{hotinfo.zTitle}}</h4>
        <img :src="hotinfo.zImg" alt="" class="col-sm-3">
  ...
  import axios from 'axios'
    export default {
      ...
      methods:{
        ...
        showmoreinfo(){
          if(this.data>this.totalhotinfo.length){
            this.$message({
              showClose: true,
              message: '没有数据了！',
              type: 'success'
      ...
      mounted(){
          let _this = this;
        axios.get(_this.$store.state.url + '/information/hot')
          .then(function (result) {
            let info = eval("(" + result.request.response + ")");
            _this.totalhotinfo = info.data[0];
