<template>
  <div class="total">
    ...
    <div v-for="(otherinfo,index) in totalotherinfo" class="onediv" v-if="index<data">
      <figure>
        <h4 class="detail"  @click="tootherdetails(otherinfo.zId)"><span>| 其它 |&nbsp;&nbsp;</span>{{otherinfo.zTitle}}</h4>
        <img :src="otherinfo.zImg" alt="" class="col-sm-3">
        <figcaption class="col-sm-9">
          <article>
            <header>
              <hgroup>
                <p>{{otherinfo.zContent}}</p>
              </hgroup>
            </header>
            <div class="span">
              <span class="glyphicon glyphicon-time" aria-hidden="true"></span>
              <span>{{otherinfo.zTime.substring(0,10)}}&nbsp;&nbsp;</span>
              <span>{{otherinfo.zFrom}}</span>
            </div>
          </article>
        </figcaption>
        <hr class="hr">
  ...
  import axios from 'axios'
    export default {
      ...
      methods:{
        ...
        showmoreinfo(){
          if(this.data>this.totalotherinfo.length){
            this.$message({
              showClose: true,
              message: '没有数据了！',
              type: 'success'
      ...
      mounted(){
        let _this = this;
        axios.get(_this.$store.state.url + '/information/other')
          .then(function (result) {
              let info = eval("(" + result.request.response + ")");
              _this.totalotherinfo = info.data[0];
